...
#   two options.
cps_aim_org = choice_dict[choice]['Organization Variable AIM']
...
cps_aim_df = cps_aim_df.dropna(thresh=20)
...
# Role variables
role_dictionary_aim = meta_dict(meta_df, 'role')
role_count_aim = cps_aim_df['role'].value_counts().to_dict()
role_survey_count_aim = cps_aim_df['role'].count()
...
training = cps_aim_df[training_columns]
...
' + choice_dict[choice]['type'] + ', job role, and CPS training status as \
follows:\n'
)
